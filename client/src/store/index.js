...
function GlobalStoreContextProvider(props) {
    ...
    // store.sortIdNamePairs = (idNamePairs, sortCriteria) => {
    //     let sortedIdNamePairs = idNamePairs.slice();
        // if (sortCriteria === 'By Creation Date (Old-New)') {
        ...
        //         return 0;
        //     })
        // } else if (sortCriteria === 'By Last Edit Date (New-Old)') {
        //     sortedIdNamePairs = sortedIdNamePairs.sort((l1, l2) => {
        ...
        //         return 0;
        //     })
        // } else if (sortCriteria === 'By Name (A-Z)' || sortCriteria === 'Name (A-Z)') {
        //     sortedIdNamePairs = sortedIdNamePairs.sort((l1, l2) => {
        //         if (l1.name < l2.name) {
        //             return -1;
        //         }
        ...
        //         return 0;
        //     })
        // } else if (sortCriteria === 'Publish Date (Newest)') {

        // } else if (sortCriteria === 'Listens (High - Low)') {

        // } else if (sortCriteria === 'Likes (High - Low)') {
        // } else if (sortCriteria === 'Dislikes (High - Low)') {
        // }   
    //     return sortedIdNamePairs
    // }

    store.setIdNamePairs = (idNamePairs) => {
        storeReducer({
    ...
    store.searchByQuery = () => {
        if (store.searchQuery === null || store.searchQuery.trim() === '') {
            storeReducer({
                type: GlobalStoreActionType.LOAD_ID_NAME_PAIRS,
                payload: []
            });
        ...
        async function asyncLoadPlaylistsByQuery() {
            console.log('id what')
            const response = store.currentScreen === ScreenNames.ALL ?
                await api.searchPlaylists(store.searchQuery, 'ALL', null)
                : await api.searchPlaylists(store.searchQuery, 'USERS', null);
            console.log('id what')
        ...
        asyncLoadPlaylistsByQuery();
    }

    store.setSortCriteria = (criteria) => {
        storeReducer({
    ...
    store.duplicateList = async function (list) {
        ...
        if (response2.data.success) {
            let pairsArray = response2.data.data;
            
            pairsArray.forEach(list => {
                if (list.name === newListName) {
                    newListName = newListName + '100'
                }
            })
